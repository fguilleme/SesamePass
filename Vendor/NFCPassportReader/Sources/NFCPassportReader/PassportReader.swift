...
public class PassportReader : NSObject {
    ...
    public func readPassport( mrzKey : String, tags : [DataGroupId] = [], aaChallenge: [UInt8]? = nil, skipSecureElements : Bool = true, skipCA : Bool = false, skipPACE : Bool = false, useExtendedMode : Bool = false, usePACEDiscovery: Bool = false, customDisplayMessage : ((NFCViewDisplayMessage) -> String?)? = nil) async throws -> NFCPassportModel {
        ...
        return try await withCheckedThrowingContinuation { (continuation: NFCCheckedContinuation) in
            ...
            let polling: NFCTagReaderSession.PollingOption
            if usePACEDiscovery {
                if #available(iOS 26.4, *) {
                    polling = [.iso14443, .pace]
                } else if #available(iOS 16.0, *) {
                    // Before 26.4 PACE polling cannot be combined with ISO14443.
                    polling = [.pace]
                } else {
                    polling = [.iso14443]
                }
            ...
            self.readerSession = NFCTagReaderSession(pollingOption: polling, delegate: self, queue: nil)
    ...
    func startReading(tagReader : TagReader) async throws -> NFCPassportModel {
        ...
        if passport.PACEStatus != .success {
            do {
                ...
                // Preserve the PACE cause when BAC cannot recover (notably PACE-only cards).
                throw paceFailure ?? error
            }
        }
        ...
        repeat {
            do {
                ...
                if errMsg == "Session invalidated" || errMsg == "Class not supported" || errMsg == "Tag connection lost" || errMsg == "Tag response error / no response" {
                    ...
                    self.dataGroupsToRead.removeAll { $0 == dgId }
                    redoBAC = true
